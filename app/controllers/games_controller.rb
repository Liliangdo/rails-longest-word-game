...
class GamesController < ApplicationController
  ...
  def score
    @mychoice = params[:query].upcase
    @attempt = params[:query]
    @submit_time = Time.parse params[:submit_time]
    @time_taken = (Time.now - @submit_time).to_i
    @my_score = compute_score(@attempt, @time_taken).truncate
    @grid = params[:grid].chars
    @result = included?(@guess, @grid)
  end
end
